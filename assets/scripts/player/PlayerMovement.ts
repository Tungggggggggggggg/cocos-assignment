...
export class PlayerMovement extends Component {
    ...
    protected onEnable(): void {
        // Lắng nghe Vec2 từ một event riêng nếu cần,
        // hoặc đơn giản là share _moveDir qua node component reference
    ...
    protected update(dt: number): void {
        ...
        const newX = math.clamp(pos.x + this._moveDir.x * dist, this._minBound.x, this._maxBound.x);
        const newY = math.clamp(pos.y + this._moveDir.y * dist, this._minBound.y, this._maxBound.y);

        this._tempPos.set(newX, newY, 0);
        this.node.setPosition(this._tempPos);

        if (this._moveDir.x !== 0) {
            const sx = this._moveDir.x > 0
